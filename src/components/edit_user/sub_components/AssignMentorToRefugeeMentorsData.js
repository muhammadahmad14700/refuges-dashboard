...
function AssignMentorToRefugeeMentorsData(props) {
  ...
  React.useEffect(() => {
    ...
    if (assignedMunicipalities && assignedMunicipalities.length > 0) {
      ...
      if (props.searchText) {
        dispatch(Actions.getAllMiniSearchedMentors(0 + 1, rowsPerPage, props.searchText, sessionStorage.getItem("user_id"), municipalityId));
      }
      else {
        dispatch(Actions.getAllMiniSearchedMentors(page + 1, rowsPerPage, props.searchText, sessionStorage.getItem("user_id"), municipalityId));
      ...
      history.push({
        pathname: "/editAssignMunicipalityToRefugee"
      });
    }
  ...
  const handleChangeSelected = (index) => {
    ...
    if (newObj.docs[index].isSelected && !newObj.docs[index].isAssigned) {
      ...
      if (props.assignedMentorsState.length > 0) {

        props.setUnSelectedMentors(props.assignedMentorsState);
      }
    ...
    if (newObj.docs[index].isSelected && newObj.docs[index].isAssigned) {
      ...
      props.setAssignedMentorsState([]);
      props.setSelectedMentors([newElement]);
      if (props.unSelectedMentors.length > 0) {
        props.setUnSelectedMentors(props.unSelectedMentors.filter(item => item.id !== newObj.docs[index].id));
    ...
    if (!newObj.docs[index].isSelected && !newObj.docs[index].isAssigned) {
      ...
      props.setAssignedMentorsState(props.assignedMentorsState.filter(item => item.id !== newObj.docs[index].id));
    }
    if (!newObj.docs[index].isSelected && newObj.docs[index].isAssigned) {
      let newElement = {
        bsn: newObj.docs[index].bsn,
      ...
      props.setUnSelectedMentors([newElement]);
      props.setSelectedMentors(props.selectedMentors.filter(item => item.id !== newObj.docs[index].id));
      props.setAssignedMentorsState(props.assignedMentorsState.filter(item => item.id !== newObj.docs[index].id));
    }
    setMentors(newObj);
  ...
  const SearchedMentors = useSelector(
    ({ AssignMentorToRefugeeMentorsDataReducer }) =>
      AssignMentorToRefugeeMentorsDataReducer.allMiniSearchedMentorsReducer.data
  );
  const loading = useSelector(
    ({ AssignMentorToRefugeeMentorsDataReducer }) =>
      AssignMentorToRefugeeMentorsDataReducer.allMiniSearchedMentorsReducer.isLoading
  );

  const errMsg = useSelector(
    ({ AssignMentorToRefugeeMentorsDataReducer }) =>
      AssignMentorToRefugeeMentorsDataReducer.allMiniSearchedMentorsReducer.errMsg
  );
  React.useEffect(() => {
    timer.current = window.setTimeout(() => {
      ...
      if (SearchedMentors && SearchedMentors.miniSearchAllMentors && (props.selectedMentors.length > 0 || props.unSelectedMentors.length > 0)) {
          ...
          for (let i = 0; i < selmentorsObj.docs.length; i++) {
            ...
            if (props.unSelectedMentors.length > 0) {
              for (let j = 0; j < props.unSelectedMentors.length; j++) {
                const selMunicipality = props.unSelectedMentors[j];
                if (selMunicipality.isAssigned) {
                  seldoc["isSelected"] = false;
                  props.setAssignedMentorsState(props.assignedMentorsState.filter(item => item.id !== selMunicipality.id));
    ...
    <div>
      <div className="system-search-filter-div">
        <div className="system-search-filter-total-div">
          Total Mentors: {(mentors && mentors.totalDocs) ? mentors.totalDocs : "0"}
        </div>
...
export default withReducer("AssignMentorToRefugeeMentorsDataReducer", reducer)(AssignMentorToRefugeeMentorsData);