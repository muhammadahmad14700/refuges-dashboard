...
// GET ALL SEARCHED_MUNICIPLITIES
export const GET_ALL_SEARCHED_MUNICIPLITIES = "GET_ALL_SEARCHED_MUNICIPLITIES";
export const ALL_SEARCHED_MUNICIPLITIES_IS_LOADING =
  "ALL_SEARCHED_MUNICIPLITIES_IS_LOADING";
export const ALL_SEARCHED_MUNICIPLITIES_FAILED =
  "ALL_SEARCHED_MUNICIPLITIES_FAILED";
export const RESET_SEARCHED_MUNICIPLITIES = "RESET_SEARCHED_MUNICIPLITIES";

// GET ALL MINI_SEARCHED_MUNICIPLITIES
export const GET_ALL_MINI_SEARCHED_MUNICIPLITIES = "GET_ALL_MINI_SEARCHED_MUNICIPLITIES";
export const ALL_MINI_SEARCHED_MUNICIPLITIES_IS_LOADING =
  "ALL_MINI_SEARCHED_MUNICIPLITIES_IS_LOADING";
export const ALL_MINI_SEARCHED_MUNICIPLITIES_FAILED =
  "ALL_MINI_SEARCHED_MUNICIPLITIES_FAILED";
export const RESET_MINI_SEARCHED_MUNICIPLITIES = "RESET_MINI_SEARCHED_MUNICIPLITIES";

// GET ALL SEARCHED_MANAGERS
export const GET_ALL_SEARCHED_MANAGERS = "GET_ALL_SEARCHED_MANAGERS";
export const ALL_SEARCHED_MANAGERS_IS_LOADING =
  "ALL_SEARCHED_MANAGERS_IS_LOADING";
export const ALL_SEARCHED_MANAGERS_FAILED =
  "ALL_SEARCHED_MANAGERS_FAILED";
export const RESET_SEARCHED_MANAGERS = "RESET_SEARCHED_MANAGERS";
...
// GET ALL SearchedMunicipalities HELPERS
export const allSearchedMunicipalitiesIsLoading = (ms) => ({
  type: ALL_SEARCHED_MUNICIPLITIES_IS_LOADING,
  payload: ms,
});

export const allSearchedMunicipalitiesFailed = (ms) => ({
  type: ALL_SEARCHED_MUNICIPLITIES_FAILED,
  payload: ms,
});

export const resetSearchedMunicipalities = (ms) => ({
  type: RESET_SEARCHED_MUNICIPLITIES,
  payload: ms,
});

// GET ALL MiniSearchedMunicipalities HELPERS
export const allMiniSearchedMunicipalitiesIsLoading = (ms) => ({
  type: ALL_MINI_SEARCHED_MUNICIPLITIES_IS_LOADING,
  payload: ms,
});

export const allMiniSearchedMunicipalitiesFailed = (ms) => ({
  type: ALL_MINI_SEARCHED_MUNICIPLITIES_FAILED,
  payload: ms,
});

export const resetMiniSearchedMunicipalities = (ms) => ({
  type: RESET_MINI_SEARCHED_MUNICIPLITIES,
  payload: ms,
});

// GET ALL SearchedManagers HELPERS
export const allSearchedManagersIsLoading = (ms) => ({
  type: ALL_SEARCHED_MANAGERS_IS_LOADING,
  payload: ms,
});

export const allSearchedManagersFailed = (ms) => ({
  type: ALL_SEARCHED_MANAGERS_FAILED,
  payload: ms,
});

export const resetSearchedManagers = (ms) => ({
  type: RESET_SEARCHED_MANAGERS,
  payload: ms,
});
...
// GET AllSearchedMunicipalities  MAIN ACTION

export function getAllSearchedMunicipalities(page, limit, search = '', filter = 'all', mentorId = '', refugeeId = '', supplierId = '', status = 'all') {
  console.log(page, limit, search, filter, mentorId);
  return (dispatch) => {
    dispatch(allSearchedMunicipalitiesIsLoading(true));

    client
      .query({
        query: gql`
            {
              searchAllMunicipalities(page: ${parseInt(page)}, limit: ${parseInt(limit)}, search: "${search}", filter:{mentorId: "${mentorId}", refugeeId: "${refugeeId}", supplierId: "${supplierId}", assignType: "${filter}", status: "${status}"}) {
                totalDocs
                limit
                page
                totalPages
      ...
      .then((response) => {
        dispatch({
          type: GET_ALL_SEARCHED_MUNICIPLITIES,
      ...
      .catch((err) => {
        if (err.networkError && err.networkError.result && err.networkError.result.errors && err.networkError.result.errors.length && err.networkError.result.errors[0] && err.networkError.result.errors[0].extensions && err.networkError.result.errors[0].extensions.code && err.networkError.result.errors[0].extensions.code === 401) {
          ...
          if (err.graphQLErrors && err.graphQLErrors.length > 0) {
            dispatch(
              allSearchedMunicipalitiesFailed(
...
export function getAllMiniSearchedMunicipalities(page, limit, search = '', mentorId = '', refugeeId = '', supplierId = '') {
  return (dispatch) => {
    dispatch(allMiniSearchedMunicipalitiesIsLoading(true));

    client
      .query({
        query: gql`
          {
            miniSearchAllMunicipalities(page: ${parseInt(page)}, limit: ${parseInt(limit)}, search: "${search}", filter:{mentorId: "${mentorId}", refugeeId: "${refugeeId}", supplierId: "${supplierId}"}) {
              totalDocs
              limit
              page
              totalPages
      ...
      .then((response) => {
        dispatch({
          type: GET_ALL_MINI_SEARCHED_MUNICIPLITIES,
          payload: response.data,
        });
      })
      ...
      .catch((err) => {
        if (err.networkError && err.networkError.result && err.networkError.result.errors && err.networkError.result.errors.length && err.networkError.result.errors[0] && err.networkError.result.errors[0].extensions && err.networkError.result.errors[0].extensions.code && err.networkError.result.errors[0].extensions.code === 401) {
          ...
          if (err.graphQLErrors && err.graphQLErrors.length > 0) {
            dispatch(
              allMiniSearchedMunicipalitiesFailed(
                err.graphQLErrors[0].message
              )
            );
          }
...
export function getAllSearchedManagers(page, limit, search = '', filter = 'all', status = 'all') {
  return (dispatch) => {
    dispatch(allSearchedManagersIsLoading(true));

    client
      .query({
        query: gql`
            {
                searchAllManagers(page: ${parseInt(page)}, limit: ${parseInt(limit)}, search: "${search}", filter:{assignType: "${filter}", status: "${status}"}) {
                    totalDocs
                    limit
                    page
                    totalPages
      ...
      .then((response) => {
        dispatch({
          type: GET_ALL_SEARCHED_MANAGERS,
          payload: response.data,
        });
      })
...
export function getAllSearchedMentors(page, limit, search = '', filter = 'all', refugeeId = '', status = 'all', municipalityId = '') {
  return (dispatch) => {
    ...
    client
      .query({
        query: gql`
          {
            searchAllMentors(page: ${parseInt(page)}, limit: ${parseInt(limit)}, search: "${search}", filter:{refugeeId: "${refugeeId}", assignType: "${filter}", status: "${status}", municipalityId: "${municipalityId}"}) {
                  totalDocs
                  limit
                  page
                  totalPages
...
export function getAllSearchedSuppliers(page, limit, search = '', filter = 'all', type = '', refugeeId = '', status = 'all', municipalityId = '') {
  return (dispatch) => {
    ...
    client
      .query({
        query: gql`
        {
          searchAllSuppliers(page: ${parseInt(page)}, limit: ${parseInt(limit)}, search: "${search}", filter:{refugeeId: "${refugeeId}", assignType: "${filter}", supplierType: "${type}", status: "${status}", municipalityId: "${municipalityId}"}) {
                totalDocs
                limit
                page
                totalPages
