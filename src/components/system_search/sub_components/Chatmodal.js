...
export default function Chatmodal(props) {
  ...
  function join() {
    socket.current.emit('join', {
      receiverId: (valueState === 0 && props.data.id) ? props.data.id : ""
    }, (res) => {
        ...
        >
          <div className="chat-div">
            <AppBar position="static" style={{ height: "fit-content", width: "fit-content", boxShadow: "none", backgroundColor: "white", color: "black" }}>
              <Tabs TabIndicatorProps={{
                ...
                <Tab
                  ...
                  label={<span className={classes.tabLabel}>Manager</span>} {...a11yProps(0)} />
              </Tabs>
            </AppBar>
            <TabPanel value={value} index={0}>
                  ...
                  <div id="scrollableDiv" ref={paneDidMount} className="msg-wrapper">
                    {
                      ...
                      messages.map((msg) => (
                        ...
                        (msg.type === "text") ? (<div className={(msg.senderId === jwt.decode(localStorage.jwtToken).id) ? "chat-receiver sb12" : "chat-sender sb11"}>
                          <h5 className="msg-head">{(msg.senderId === jwt.decode(localStorage.jwtToken).id) ? "Admin" : props.data.name} {(new Date(msg.sentTime)).toLocaleString()}</h5>
              ...
              {!props.data.id && (
                <div style={{ padding: "100px" }}>Manager ID is not defined</div>
              )}
            </TabPanel>
          </div>
        </DialogContent>
