...
function ManagersData(props) {
  ...
  React.useEffect(() => {
    if (props.searchText) {
      dispatch(Actions.getAllSearchedManagers(0 + 1, rowsPerPage, props.searchText, confirmedFilter.id, confirmedStatus.id));

    } else {
      dispatch(Actions.getAllSearchedManagers(page + 1, rowsPerPage, props.searchText, confirmedFilter.id, confirmedStatus.id));
  ...
  const SearchedManagers = useSelector(
    ({ ManagersDataReducer }) =>
      ManagersDataReducer.allSearchedManagersReducer.data
  );
  const loading = useSelector(
    ({ ManagersDataReducer }) =>
      ManagersDataReducer.allSearchedManagersReducer.isLoading
  );

  const errMsg = useSelector(
    ({ ManagersDataReducer }) =>
      ManagersDataReducer.allSearchedManagersReducer.errMsg
  );
  ...
  const viewEditSummary = (data) => {
    if (data && data.municipality && data.municipality.length > 0 && data.municipality[0]._id) {
      sessionStorage.setItem("user_id", data.municipality[0]._id);
      sessionStorage.setItem("entity_name", data.municipality[0].name);
      history.push({
        pathname: "/ViewEditMunicipalitySummary"
      });
    }
    else {
      alert(t("Municipality is not assigned to this manager"))
    }
    ...
    <div>
      ...
      <div className="system-search-filter-div">
        <div className="system-search-filter-total-div">
          {t("Total Managers")}: {(SearchedManagers &&
            SearchedManagers.searchAllManagers &&
            SearchedManagers.searchAllManagers.totalDocs) ? SearchedManagers.searchAllManagers.totalDocs : "0"}
        </div>
      ...
      <div className="system-search-table-div">
        ...
        >
          ...
          <tr onClick={() => handleTooltipClose()}>
            ...
            <th style={{ textAlign: "center", paddingLeft: "0px" }}>{t("Chat")}</th>
          ...
          {SearchedManagers &&
            SearchedManagers.searchAllManagers &&
            SearchedManagers.searchAllManagers.docs !== null &&
            SearchedManagers.searchAllManagers.docs.map((doc) => (
              <tr>
                ...
                <td onClick={() => handleTooltipClose()}>
                  {doc.municipality !== null && doc.municipality.map((mun) => (
                    <Tooltip classes={classess} title={mun.name} arrow>
                      <span>{Truncate(mun.name)}</span>
                    </Tooltip>
                  ))}
                </td>
                ...
                <td style={{ textAlign: "center", paddingLeft: "0px" }} onClick={() => handleTooltipClose()}>
                  <img
                    src={chatIcon}
...
export default withReducer("ManagersDataReducer", reducer)(ManagersData);