...
function MentorForm(props) {
  ...
  React.useEffect(() => {
    dispatch(Actions.resetMentorSummary(true));
  ...
  const add_confirmation = useSelector(
    ({ MentorFormReducer }) => MentorFormReducer.AddNewMentorReducer
  );
  ...
  const {
    ...
    initialValues: {
      ...
      mentor_phone_no: (mentorSummary && mentorSummary.getMentorSummary) ? mentorSummary && mentorSummary.getMentorSummary.phoneNumber : "+31",
    },
    validationSchema,
  ...
  React.useEffect(() => {
    if (add_confirmation.data && add_confirmation.data.data && add_confirmation.data.data.addMentor && !skip) {
      let finalSteps = steps;
      finalSteps.push(props.selectedUserType);
      dispatch(Actions.resetAddNewMentor(true));
      sessionStorage.setItem("steps", JSON.stringify(finalSteps));
      sessionStorage.setItem("user_id", add_confirmation.data.data.addMentor.id);
      ...
      <form onSubmit={handleSubmit}>
        <Grid container spacing={2} className={classes.con}>
          ...
          <Grid item sm={10} xs={8}>
            <TextField
              ...
              variant="outlined"
              size="small"
              placeholder={t("Enter Mentor’s Full Name")}
        ...
        <div className="create-user-skip-div">
          <Button
            ...
            size="small"
            onClick={() => skipClick()}
            disabled={sessionStorage.getItem("user_id") ? false : true}
            classes={{
