...
function MunicipalityForm(props) {
  ...
  const add_confirmation = useSelector(
    ({ MunicipalityFormReducer }) => MunicipalityFormReducer.AddNewMunicipalityReducer
  );
  ...
  const validationSchema = yup.object({
    ...
    manager_phone_no: yup.string().required(t("Manager Phone Number is required")).matches(phoneRegExp, t('Phone Number is not valid')).min(10, t("to short"))
      .max(15, "to long"),
    manager_email: yup.string().email().required(t("Email is Required")).max(30, t("Email must be at most 30 characters")),
    contact_person_name: yup.string().required(t("Contact Person Name is required")).matches(/^[A-Za-z ]*$/, t('Please enter valid name'))
  ...
  const {
    ...
    initialValues: {
      ...
      manager_phone_no: (municipalitySummary && municipalitySummary.getMunicipalitySummary && municipalitySummary.getMunicipalitySummary.manager) ? municipalitySummary.getMunicipalitySummary.manager.phoneNumber : "+31",
    ...
    onSubmit(values) {
      if (sessionStorage.getItem("user_id")) {
        ...
        dispatch(Actions.addNewMunicipality(values));
      }
    },
  });
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    if (add_confirmation.data && add_confirmation.data.data && add_confirmation.data.data.addMunicipality && !skip) {
      let finalSteps = steps;
      finalSteps.push(props.selectedUserType);
      sessionStorage.setItem("steps", JSON.stringify(finalSteps));
      sessionStorage.setItem("user_id", add_confirmation.data.data.addMunicipality.id);
      sessionStorage.setItem("entity_name", values.municipality_name);
      dispatch(Actions.resetAddNewMunicipality(true));
      history.push({
        pathname: "/assignMentorToMunicipality"
      });
    }
    if (add_confirmation.data && add_confirmation.data.data && add_confirmation.data.data.addMunicipality && skip) {
      let finalSteps = steps;
