      ...
      .catch((err) => {
        if (
          ...
          if (err.graphQLErrors && err.graphQLErrors.length > 0) {
            ...
            dispatch(assignUnassignRolesToMentorFailed(JSON.stringify(err)));
          }
        }
      });
  };
    ...
    client
      ...
      .catch((err) => {
        if (
          ...
          if (err.graphQLErrors && err.graphQLErrors.length > 0) {
            dispatch(
              addNewMentorFailed(JSON.stringify(err.graphQLErrors[0].message))
            );
          } else {
            dispatch(
    ...
    client
      ...
      .catch((err) => {
        if (
          ...
          if (err.graphQLErrors && err.graphQLErrors.length > 0) {
            dispatch(
              updateMentorFailed(JSON.stringify(err.graphQLErrors[0].message))
            );
          } else {
        ...
        if (
          ...
          if (err.graphQLErrors && err.graphQLErrors.length > 0) {
            if (err.graphQLErrors[0].extensions && err.graphQLErrors[0].extensions.code && err.graphQLErrors[0].extensions.code === 400 && err.graphQLErrors[0].extensions.errors && err.graphQLErrors[0].extensions.errors.length > 0) {
              ...
              dispatch(
                updateSupplierFailed(JSON.stringify(err.graphQLErrors[0].message))
              );
            }
          } else {
