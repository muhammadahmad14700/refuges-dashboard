...
function Statusmodal(props) {
  ...
  React.useEffect(() => {
    dispatch(Actions.getExtraattributes("RF-4"));
  }, [dispatch]);
  ...
  const {
    ...
    onSubmit(values) {
      if (editclick && oldextraattribute) {
        dispatch(Actions.updateExtraAttribute("RF-4", values));
      } else {
        dispatch(Actions.addNewExtraAttribute("RF-4", values.extra_attribute));
      }
    },
  });
  React.useEffect(() => {
  ...
  const handleClickDelete = (data) => {
    dispatch(Actions.deleteExtraAttribute("RF-4", data));
  };
    ...
    <div>
      ...
      >
        ...
        >
          ...
          >
            Extra voor statushouders in deze gemeente
          </Typography>
        </DialogTitle>
        <DialogContent
        >
          ...
          >
            {extraattributes_confirmation &&
              extraattributes_confirmation.getExtraAttributes &&
              extraattributes_confirmation.getExtraAttributes
                .extraAttributes !== null &&
              extraattributes_confirmation.getExtraAttributes.extraAttributes.map(
                (doc) => (
          ...
          <p className="add-persoon1">+ Toevoegen</p>
          {editclick === false && (
