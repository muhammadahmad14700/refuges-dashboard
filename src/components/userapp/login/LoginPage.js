...
function LoginPage(props) {
  ...
  const confirmation = useSelector(
    ({ AuthChildReducer }) => AuthChildReducer.AuthenticateChildUserReducer
  );
  const VerifyOtpReducer = useSelector(
    ({ AuthChildReducer }) => AuthChildReducer.VerifyOtpReducer
  );

  const ResendOtpReducer = useSelector(
    ({ AuthChildReducer }) => AuthChildReducer.ResendOtpReducer
  );
  ...
  React.useEffect(() => {
    if (
      ...
      confirmation.user &&
      confirmation.user.role === "manager"
    ) {
      // history.push({
    ...
    if (
      ...
      confirmation.isOtpVerified &&
      confirmation.user &&
      confirmation.user.role === "mentor"
    ) {
      // history.push({
      //   pathname: "/mentordashboard"
      // });
      window.location.replace("/mentordashboard");
    ...
    if (
      ...
      confirmation.isOtpVerified &&
      confirmation.user &&
      (confirmation.user.role === "work_supplier" || confirmation.user.role === "school_supplier")
    ) {
      window.location.replace("/supplierdashboard");
      // history.push({
    ...
    <div>
      {otpopen === true && (
        <Otpmodalfinal
          ...
          OTPresend={OTPresend}
          isLoading={VerifyOtpReducer.isLoading}
          errMsg={VerifyOtpReducer.errMsg}
            ...
            >
              ...
              <div className="form-inside-div">
                ...
                <Link className={classes.link}
                  onClick={() => {
                    history.push({
                      pathname: "/requestResetPasswordForUser"
                    });
                  }}
                >
                  <p
...
export default withReducer("AuthChildReducer", reducer)(LoginPage);
