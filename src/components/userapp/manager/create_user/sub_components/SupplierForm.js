...
function SupplierForm(props) {
  ...
  const [selectedSupplierType, setSelectedSupplierType] = React.useState("SCHOOL");
  ...
  const {
    ...
    initialValues: {
      ...
      supplier_phone_no: (supplierSummary && supplierSummary.getSupplierSummary) ? supplierSummary && supplierSummary.getSupplierSummary.phoneNumber : "",
  ...
  React.useEffect(() => {
    if (add_confirmation.data && add_confirmation.data.data && add_confirmation.data.data.addSupplier && !skip) {
      ...
      history.push({
        pathname: "/assignRefugeeToSupplierByManager"
      });
    }
    if (add_confirmation.data && add_confirmation.data.data && add_confirmation.data.data.addSupplier && skip) {
      let finalSteps = steps;
      ...
      sessionStorage.setItem("entity_name", values.supplier_name);
      history.push({
        pathname: "/postSupplierDetailsByManager"
      });
    }
