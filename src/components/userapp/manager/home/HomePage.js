...
function HomePage(props) {
  ...
  const profile_data = useSelector(
    ({ HomePageManagerReducer }) => HomePageManagerReducer.adminProfileReducer.data
  );
  const loadingg = useSelector(
    ({ HomePageManagerReducer }) => HomePageManagerReducer.adminProfileReducer.isLoading
  );
  const errMsg = useSelector(
    ({ HomePageManagerReducer }) => HomePageManagerReducer.adminProfileReducer.errMsg
  );
  ...
  function FormRowInside(props) {
    return (
      <React.Fragment>
        <Grid item sm={8} xs={8}>
          <Paper onClick={() => Openmunipage(props.doc.id)} className={classes.paper1}>
            {props.doc.name}
          </Paper>
        </Grid>
        <Grid item sm={2} xs={2}>
  ...
  function TableRow() {
    ...
    const AssignedMunicipalities = useSelector(
      ({ HomePageManagerReducer }) =>
        HomePageManagerReducer.AllAssignedMunicipalitiesReducer.data
    );
    const loading = useSelector(
      ({ HomePageManagerReducer }) =>
        HomePageManagerReducer.AllAssignedMunicipalitiesReducer.isLoading
    );

    const errMsg = useSelector(
      ({ HomePageManagerReducer }) =>
        HomePageManagerReducer.AllAssignedMunicipalitiesReducer.errMsg
    );
    return (
      <React.Fragment>
      ...
      <Container fixed className={classes.con}>
        <Grid container spacing={3}>
          ...
          <Grid item sm={12} md={4} xs={12} spacing={3} className={classes.row1}>
            ...
            >
              {t("Welcome")} {(profile_data && profile_data.getManagerProfile && profile_data.getManagerProfile.name) ? profile_data.getManagerProfile.name : "Manager"}
            </Typography>
          </Grid>
...
export default withReducer("HomePageManagerReducer", reducer)(HomePage);
