...
function Header(props) {
  ...
  return (
    <div className={classes.root}>
      {chatopen === true && (
        <Chatmodal status={chatopen} methodd={setChatopen} />
      )}
          ...
          <div className={classes.parentDiv}>
            {list.map((doc, firstIndex) => (
              (!doc.list && doc.hasChild) ?
                ...
                :
                (!doc.list && !doc.hasChild) ?
                  <div className={classes.headDivWithNoChild} onClick={() => handleNavigate(doc.path, doc.id)}>
                    {doc.headTitle}
                  </div>
                  :
                  <>
