...
export default function Chatmodal(props) {
  ...
  function join() {
    socket.current.emit('join', {
      receiverId: (valueState === 0 && props.data.mentor) ? props.data.mentor._id : (valueState === 1 && props.data.municipality && props.data.municipality.manager) ? props.data.municipality.manager._id : (valueState === 2 && props.data.id) ? props.data.id : ""
    }, (res) => {
            ...
            <AppBar position="static" className={classes.head}>
              <Tabs TabIndicatorProps={{
                ...
                <Tab
                  ...
                  label={<span className={classes.tabLabel}>{t("Refugee")}</span>} {...a11yProps(2)} />
              </Tabs>
            </AppBar>
                  ...
                  <div id="scrollableDiv" ref={paneDidMount} className="msg-wrapper">
                    {
                      ...
                      messages.map((msg) => (
                        ...
                        (msg.type === "text") ? (<div className={(msg.senderId === jwt.decode(localStorage.jwtToken).id) ? "chat-receiver sb12" : "chat-sender sb11"}>
                          <h5 className="msg-head">{(msg.senderId === jwt.decode(localStorage.jwtToken).id) ? "Supplier" : props.data.mentor.name} {(new Date(msg.sentTime)).toLocaleString()}</h5>
                  ...
                  <div id="scrollableDiv" ref={paneDidMount} className="msg-wrapper">
                    {
                      ...
                      messages.map((msg) => (
                        ...
                        (msg.type === "text") ? (<div className={(msg.senderId === jwt.decode(localStorage.jwtToken).id) ? "chat-receiver sb12" : "chat-sender sb11"}>
                          <h5 className="msg-head">{(msg.senderId === jwt.decode(localStorage.jwtToken).id) ? "Supplier" : props.data.municipality.manager.name} {(new Date(msg.sentTime)).toLocaleString()}</h5>
                          <p className="msg-text">{msg.body}</p>
                          <div className={(msg.senderId === jwt.decode(localStorage.jwtToken).id && msg.status === "seen") ? "seen-msg-div" : "seen-msg-div-none"}>
                            <span style={{ margin: "3px", float: "right", fontSize: "12px" }}>{(new Date(msg.sentTime)).toLocaleString()}</span>
                            <DoneAllIcon style={{ color: "#3f51b5", margin: 0, float: "right" }} />
                  ...
                  <div id="scrollableDiv" ref={paneDidMount} className="msg-wrapper">
                    {
                      ...
                      messages.map((msg) => (
                        ...
                        (msg.type === "text") ? (<div className={(msg.senderId === jwt.decode(localStorage.jwtToken).id) ? "chat-receiver sb12" : "chat-sender sb11"}>
                          <h5 className="msg-head">{(msg.senderId === jwt.decode(localStorage.jwtToken).id) ? "Supplier" : props.data.name} {(new Date(msg.sentTime)).toLocaleString()}</h5>
                          <p className="msg-text">{msg.body}</p>
                          <div className={(msg.senderId === jwt.decode(localStorage.jwtToken).id && msg.status === "seen") ? "seen-msg-div" : "seen-msg-div-none"}>
                            <span style={{ margin: "3px", float: "right", fontSize: "12px" }}>{(new Date(msg.sentTime)).toLocaleString()}</span>
                            <DoneAllIcon style={{ color: "#3f51b5", margin: 0, float: "right" }} />
                          ...
                          : (msg.type === "image") ? (
                            <div className={(msg.senderId === jwt.decode(localStorage.jwtToken).id) ? "chat-receiver-map-div" : "chat-sender-map-div"}>
                              <img src={msg.body} alt="Map" className={(msg.senderId === jwt.decode(localStorage.jwtToken).id) ? "chat-receiver-map" : "chat-sender-map"} />
                            </div>)
                            : (null)
                      ))}
                    <div style={{ float: "left", clear: "both" }}
